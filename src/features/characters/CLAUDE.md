...
6. **Character slot cap** — Nexon generally adds +1 to the max characters per world with each new class. Bump both `MAX_CHARACTERS_PER_WORLD` (`model/constants.ts`) and `IP_REQUESTS_PER_DAY_LIMIT`'s fallback (`src/app/api/characters/lookup/route.ts`) together, confirming the new number in-game first.
