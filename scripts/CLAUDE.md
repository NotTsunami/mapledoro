...
- `generate-bgm-guesser-data.mjs` → `src/features/games/bgm-guesser/puzzle-data.generated.ts`. Combines `manifests/v271/bgm.json` with a hand-curated track→answer allowlist in the script, fetching [maplebgm-db](https://github.com/maplestory-music/maplebgm-db) at generation time for titles. Re-run only when the curation changes; it exits non-zero if a curated track or answer icon no longer resolves. New BGMs are *not* picked up automatically, by design.
