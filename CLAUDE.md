...
**Tool storage:** per-character data (symbols, liberation, astra, hexa skills, mystic frontier, exp calculator) lives in each character's `tools` field in the character store (`mapledoro_characters_store_v1`) via `characterToolStorage.ts`. Global data (dailies, event planner, boss crystals, trace restoration) lives under `mapledoro_tools_v1` via `globalToolsStore.ts`. Game results (Mapledle, BGM Guesser) share `mapledoro_games_v1`, one section per game, owned by `games/gamesStore.ts` — game modules go through `readGameSection`/`writeGameSection` and never touch the key directly. The Drop Tracker has its own `mapledoro_drop_tracker_v1` (an uncapped event log, deliberately kept out of the shared blob — see its feature doc).
...
Game art comes from the self-hosted **MapleResource API** (`haku.network`), via pure id→URL components in `src/components/ResourceImage.tsx` (`src/lib/mapleResource.ts`): `<ItemIcon>`, `<MobSprite>`, `<SkillIcon>`, `<HexaSkillIcon>`, `<ErdaSkillIcon>`, `<MarkIcon>` (the `ui-mark` namespace, used for BGM Guesser's answer icons). Host = `NEXT_PUBLIC_RESOURCE_BASE`; new hosts go in `next.config.mjs` `remotePatterns`.
...
- **After bumping, actually re-run every generator against the new manifest** — pointing a generator at the new version isn't the same as regenerating its output, and a stale `public/data/`/`*.generated.ts`/`*Data.ts` file gives no error, it just silently keeps serving pre-patch content. At minimum: `gen-equipment.mjs` (needs `EQUIP_ICON_DIR` + `EQUIP_DEDUP_VERDICTS` for dedup, see its own doc comment), `gen-familiars.mjs` (needs `FAMILIAR_DUMP_DIR` for reissue dedup — running without it risks reintroducing duplicate picker rows), `gen-vmatrix.mjs`, `gen-stat-baselines.mjs`. Diff the output before committing; a same-id-count regen can still hide real stat/name changes worth a second look. **The daily games' generators are excluded from this sweep** — regenerating a puzzle payload reshuffles the daily order and breaks in-flight streaks, so bumping their manifest path is not a reason to re-run them (Mapledle's payload is still v269 on purpose; see its feature doc).
...
- `scripts/scrape-class-resources.mjs` → `src/app/guides/character-guides/classResources.ts` (consumed by the `[className]` guide page). Per-class community links (Discord, docs) pulled from a third-party class-reference site's published data (the source URL lives in the script). A one-shot scraper: re-run only when a class's community resources change or a new class needs an entry, not on a version bump. Its `CLASS_PATHS` table maps our class names to that source's slugs, so a new class needs a row there first.
